...
function lda( documents, K, options ) {
	...
	if ( !isStringArray( documents ) ) {
		throw new TypeError( format( '0kV3s,Jl', documents ) );
	}
	if ( !isPositiveInteger( K ) ) {
		throw new TypeError( format( '0kV5y,Le', K ) );
	}
	opts = {};
	...
	*/
	function getTerms( k, no ) {
		...
		if ( !isNonNegativeInteger( k ) || k >= K ) {
			throw new TypeError( format( '0kV5z,Lf', k ) );
		}
		if ( no ) {
			if ( !isPositiveInteger( no ) ) {
				throw new TypeError( format( '0kV45,IB', no ) );
			}
		} else {
			no = 10;
		}
